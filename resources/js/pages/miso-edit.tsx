...
export default function MisoEdit({ request, tab }: Props) {
    const [extractedData, setExtractedData] = useState<Partial<MisoAccomplishment> | null>(null);

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: `Edit MISO Record ${request.record_no || request.id}`, href: `/miso-accomplishments/${request.id}/edit` },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit MISO Record - ${request.record_no || request.id}`} />
            ...
            <div className="flex-1 space-y-12 p-3 md:p-6 lg:p-8 w-full">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 border-b border-border/50 pb-12 w-full">
                    <div className="flex items-center gap-7">
                        ...
                        <div>
                            <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tighter leading-none">Edit MISO Accomplishment</h1>
                            <p className="text-muted-foreground text-sm font-medium mt-3 w-full max-w-xl">
                                Update this record or use Snap-to-Log to re-extract values from a source file.
                            </p>
                        </div>
                    </div>
                    ...
                    <SnapToLogBanner
                        ...
                        extraFormFields={{ category: request.category }}
                    />
                </div>
                ...
                <div className="relative w-full">
                    ...
                    <MisoRequestForm request={request} category={request.category} tab={tab} isEdit={true} extractedData={extractedData} />
                </div>
            </div>
        </AppLayout>
    );
