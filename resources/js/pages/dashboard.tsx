...
export default function Dashboard({ requests, requesters, filters }: DashboardProps) {
    const [searchTerm, setSearchTerm] = useState(filters.search || '');
    const [statusFilter, setStatusFilter] = useState(filters.status || 'all');
    const [typeFilter, setTypeFilter] = useState(filters.type || 'all');
    const [requesterFilter, setRequesterFilter] = useState(filters.requester || 'all');
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            router.get('/dashboard', params, { preserveState: true, replace: true, preserveScroll: true });
        }, 500);
        return () => clearTimeout(timer);
    }, [searchTerm, statusFilter, typeFilter, requesterFilter]);
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <Select value={typeFilter} onValueChange={setTypeFilter}>
                            <SelectTrigger className="w-36 h-10 bg-card border-border text-muted-foreground uppercase tracking-wider text-[10px] font-bold">
                                <SelectValue placeholder="All Types" />
                            </SelectTrigger>
                            <SelectContent className="bg-popover border-border text-popover-foreground">
                                <SelectItem value="all">All Types</SelectItem>
                                <SelectItem value="Technical Support">Technical Support</SelectItem>
                                <SelectItem value="Network/Internet">Network/Internet</SelectItem>
                                <SelectItem value="User Account Management">User Account</SelectItem>
                                <SelectItem value="Software Install">Software Install</SelectItem>
                        ...
                        <Select value={requesterFilter} onValueChange={setRequesterFilter}>
                            <SelectTrigger className="w-48 h-10 bg-card border-border text-muted-foreground uppercase tracking-wider text-[10px] font-bold">
                            ...
                            {confirmAction.type === 'delete' && (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2 pt-2">
                                        <p className="text-xs font-medium text-foreground">Please type <span className="font-bold text-destructive">DELETE</span> to confirm</p>
                                        <Input
                                            value={deleteConfirmText}
                                            onChange={(e) => setDeleteConfirmText(e.target.value)}
                                            placeholder="Type DELETE here"
                                            className="bg-card border-border focus:border-destructive placeholder:text-muted-foreground/50 h-10"
                                        />
                                    </div>
                                </div>
                            )}
